...
# do MacPorts setup on darwin
    ...
    # zshenv.
PATH="/Users/terana/.go/bin:/usr/local/bin:/usr/local/sbin:/usr/local/opt/ruby/bin:$PATH"
MANPATH="/opt/local/share/man:$MANPATH"
export GOPATH="/Users/terana/.go"
#fi
...
${BLUE}%n${NO_COLOR}@%m %40<...<%B%~%b%<< "
        ...
        if (( EUID != 0 )); then
            PROMPT="${RED}${EXITCODE}${WHITE}${debian_chroot:+($debian_chroot)}${BLUE}%n${NO_COLOR}@%m %60<...<%B%~%b%<< ${prompt_newline}"
        else
            PROMPT="${BLUE}${EXITCODE}${WHITE}${debian_chroot:+($debian_chroot)}${RED}%n${NO_COLOR}@%m %60<...<%B%~%b%<<> ${prompt_newline}"
        fi
    fi
fi
...
#ASP Aliases
alias ft-uart='sudo kextload /System/Library/Extensions/FTDIUSBSerialDriver.kext'
alias ft-direct='sudo kextunload /System/Library/Extensions/FTDIUSBSerialDriver.kext'
...
[[ -s "/Users/terana/.gvm/bin/gvm-init.sh" ]] && source "/Users/terana/.gvm/bin/gvm-init.sh"
